...
{
    public ProductsByCategorySpec(Guid categoryId, ProductStatus? status = null)
        : base(p => p.CategoryId == categoryId && (status == null || p.Status == status))
    {
    }

    public ProductsByCategorySpec(Guid categoryId, int page, int pageSize, ProductStatus? status = null, string? sortBy = null, bool sortDescending = false)
        : base(p => p.CategoryId == categoryId && (status == null || p.Status == status))
    {
