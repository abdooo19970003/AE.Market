...
{
    public UpdateAttributeGroupCommandValidator(IReadRepository<AttributeGroup> repo)
    {
        RuleFor(x => x.Id).NotEmpty();
        RuleFor(x => x.GroupName).NotEmpty().MaximumLength(200);
        RuleFor(x => x.Slug).MaximumLength(300)
            .MustAsync(async (cmd, slug, ct) => slug is null || !await repo.AnyAsync(
                new BaseSpecification<AttributeGroup>(g => g.Slug != null && g.Slug == Domain.Aggregates.Catalog.ValueObjects.Slug.From(slug) && g.Id != cmd.Id), ct))
